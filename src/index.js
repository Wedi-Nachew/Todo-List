...
const InputFormDisplay = (() => {
    ...
    const eventHandlers = () => {
        ...
        projectInputWrapper.addEventListener("click", (event) => {
            if (
                !projectInput.contains(event.target) ||
                event.target.nodeName == "IMG"
            ) {
        ...
        const setProject = (() => {
            sideBar.childNodes.forEach((child) => {
                child.childNodes.forEach((grandChild) => {
                    if (
                        (grandChild.classList == "active" &&
                            grandChild.textContent.includes("Today")) ||
                        (grandChild.classList == "active" &&
                            grandChild.textContent.includes("Upcoming")) ||
                        (grandChild.classList == "active" &&
                            grandChild.textContent.includes("This Week")) ||
                        (grandChild.classList == "active" &&
                            grandChild.textContent.includes("Inbox"))
                    ) {
                        projectName = "Home";
                    } else if (grandChild.classList == "active") {
                        projectName = grandChild.textContent
                            .replace(/[\n]/, "")
                            .trim();
                    }
                });
        ...
        sideBar.childNodes.forEach((child) => {
            child.childNodes.forEach((grandChild) => {
                if (
                    ...
                    FilterTasks.renderFilteredTasks(
                        todayTasks.determineDueDate()
                    );
                } else if (
                    grandChild.classList == "active" &&
                    grandChild.textContent.includes("Upcoming")
                    ...
                    FilterTasks.renderFilteredTasks(
                        UpcomingTasks.determineFutureEvent()
                    );
                } else if (
                    grandChild.classList == "active" &&
                    grandChild.textContent.includes("This Week")
                    ...
                    FilterTasks.renderFilteredTasks(
                        ThisWeekTasks.determineThisWeekTasks()
                    );
                } else if (grandChild.classList == "active") {
                    FilterTasks.renderFilteredTasks(
                        Projects.projectTask(
                            grandChild.textContent.replace(/[\n]/, "").trim()
                        )
                    );
                }
            });
...
const RenderTasks = (() => {
    ...
    function removeAnyButton() {
        inputDiv.childNodes.forEach((child) =>
            child.nodeName == "BUTTON" ? inputDiv.removeChild(child) : false
        );
    }
...
const PriorityMark = (() => {
    ...
    function priorityIndicators() {
        content.childNodes.forEach((child) => {
            if (
                child.classList == "task" ||
                child.classList == "task completed"
            ) {
                child.childNodes.forEach((grandChild) => {
                    if (
                        grandChild.className == "priority" &&
                        grandChild.textContent == 1
                    ) {
                        grandChild.parentNode.style.cssText =
                            "border-left: 12px solid #ef476f;";
                    } else if (
                        grandChild.className == "priority" &&
                        grandChild.textContent == 2
                    ) {
                        grandChild.parentNode.style.cssText =
                            "border-left: 12px solid #ffd166;";
                    } else if (
                        grandChild.className == "priority" &&
                        grandChild.textContent == 3
                    ) {
                        grandChild.parentNode.style.cssText =
                            "border-left: 12px solid #06d6a0;";
                    } else if (
                        grandChild.className == "priority" &&
                        grandChild.textContent == 4
                    ) {
                        grandChild.parentNode.style.cssText =
                            "border-left: 12px solid #1b263b;";
                    }
                });
            }
        });
    ...
    function statusIndicator() {
        content.childNodes.forEach((child) => {
            if (child.className == "task") {
                child.childNodes.forEach((grandChild) => {
                    if (
                        grandChild.className == "status" &&
                        grandChild.textContent == "uncompleted"
                    ) {
                        grandChild.parentNode.classList.remove("completed");
                    } else if (
                        grandChild.className == "status" &&
                        grandChild.textContent == "completed"
                    ) {
    ...
    const eventHandlers = (() => {
        sideBar.addEventListener("click", (event) => {
            if (
                ...
                event.target.className !== "add-project" &&
                event.target.className !== "remove-projects"
            ) {
                sideBar.childNodes.forEach((child) => {
                    if (child.nodeName == "DIV") {
                        child.childNodes.forEach((grandChild) => {
                            grandChild.className == "active"
...
const manageTasks = (() => {
    ...
    function removeTask(event) {
        ...
        Tasks.tasks.forEach((task) => {
            if (
                Object.values(task).includes(
                    event.target.parentNode.firstChild.firstChild.textContent
                )
            ) {
...
const Projects = (() => {
    ...
    function removeProjects(event) {
        ...
        Tasks.tasks.forEach((task) => {
            if (
                Object.values(task)[Object.values(task).length - 1] ==
                event.target.parentNode.textContent
            ) {
                Tasks.tasks.splice(Tasks.tasks.indexOf(task), 1);
